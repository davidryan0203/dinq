	            ...
	            <div class="card">
					...
					<v-client-table v-if="orders" :data="orders" :columns="['id','sender','receiver', 'order_type','status','order_total','date_created','order_expiry']" :options="options">
                        ...
                        <template slot="order_expiry" slot-scope="props" >
                        	<span>{{props.row.order_expiry | expiryFormat}} hours</span>
                        </template>
                        <template slot="child_row" slot-scope="props">	
                        	...
                        	<table class="table">
                        		<thead>
                        			<th>Name</th>
                        			<th v-if="userDetails.user_type == '2'">Venue</th>
                        			<th>Add-ons used</th>
                        		</thead>
                        		<tbody>
                        			<tr v-for="item in props.row.menu_items">
                        				<td>{{item.name}}</td>
                        				<td v-if="userDetails.user_type == '2'">{{(item.venue) ? item.venue.user.name : 'N/A'}}</td>
							      	...
							      	<div class="modal-body">
		                                <form-wizard @on-complete="onComplete" 
								            ...
								            <tab-content title="Items" :before-change="beforeTabSwitch">
											    ...
											    <v-client-table v-if="menuItems" :data="menuItems" :columns="['id','name','venue_name','sling_price','stock_quantity','mixer','order_quantity']" :options="options">
